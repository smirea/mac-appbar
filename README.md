...
Clicking the monochrome App Store compass icon opens a compact deployment popup with the latest Xcode Cloud build number, status, branch, date, App Store build processing state when available, workflow rules, and a manual build button.
...
The App Store-style compass glyph comes from the SVG Logos `Apple App Store` icon through Iconify/shadcn, listed as CC0. The menu bar uses only the bare monochrome compass glyph, not the blue tile background.
